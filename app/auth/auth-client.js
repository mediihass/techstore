...
export default function AuthClientPage() {
  ...
  const signUp = async (email, password, name) => {
    // Always returns a user object
    return { user: { email, name, id: Date.now().toString() } };
  };
  // -----------------------------------------------------------------
  ...
  const handleSocialAuth = async (provider) => {
    ...
    try {
      ...
      // This should never happen with our modified functions
      console.error(err);
      setError(
  ...
  const handleEmailAuth = async (e) => {
    ...
    try {
      if (isSignIn) {
        // Always succeed for sign in
        await signIn(email, password);
        router.push("/dashboard");
      } else {
        // Always succeed for sign up
      ...
      <div className="space-y-3 w-full max-w-md">
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 rounded-lg bg-linear-to-br from-blue-50 to-indigo-100 text-gray-500">
              Ou bien
            </span>
          </div>
        </div>
        ...
        <div className="text-center">
          ...
          >
            {isSignIn
              ? "Vous n'avez pas de compte? S'inscire"
